...
export class GoogleClient {
  ...
  getLoginUrl = (redirectUrl: string) => {
    const oauth2Client = new google.auth.OAuth2(this.config.clientId, this.config.clientSecret, redirectUrl);
    try {
      ...
      throw new Error(
        "Failed generating Google OAuth URL: " + error.message,
      )
    }
  }
  ...
  requestGroups = async (accessToken: string, userEmail: string) => {
    ...
    if (this.config.groupsConfig) {
      const oauth2Client = new google.auth.GoogleAuth(this.buildGoogleAuthOptions(this.config.groupsConfig))
      ...
      try {
        ...
        throw new Error("Failed requesting Google user groups: " + error.message)
      }
    } else {
